        ...
        <div class="am-g">
            <div class="am-u-sm-12">
                <table class="am-table am-table-striped am-table-hover table-main">
                    ...
                    <tbody>
                    <?php
                    foreach($messages as $admin){
                        ?>
                        <tr>
                            <td><input data-id="<?php echo $admin -> message_id; ?>" class="table-input" type="checkbox" /></td>
                            <td class="admin-id"><?php echo $admin -> message_id; ?></td>
                            ...
                            <td
                                <div class="am-btn-toolbar">
                                    <div class="am-btn-group am-btn-group-xs">
                                        <button data-id="<?php echo $admin -> message_id; ?>" class="am-btn-update am-btn am-btn-default am-btn-xs am-text-secondary"><span class="am-icon-pencil-square-o"></span> 编辑</button>
                                        <button data-id="<?php echo $admin -> message_id; ?>" class="am-btn-delete am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"><span class="am-icon-trash-o"></span> 删除</button>
                                    </div>
                                </div>
                            </td>
                        </tr>
    ...
    $(function(){
        ...
        $('.am-btn-update').on('click', function(){
            ...
            if(confirm('确定是否修改记录。不可恢复？！')){
                var Content = prompt("输入修改后的留言:","");
                if(Content){
                    location.href = 'admin/update_message?message_id='+messageId + "&content=" + Content;
                }
            }
        });
        ...
        $('.am-btn-insert').on('click', function(){
            ...
            var Email = prompt("输入您的email：","");
            var Content = prompt("输入新增的留言","");
            if(userName && Content && Email){
                $.get('admin/insert_message?username='+userName + "&email=" + Email + "&content=" + Content,function(res){
                    if(res){
                        location.href = 'admin/admin_message';
                    }
                });
            }
        });
...
//           console.log($(this).data('id'));
                ...
                for(var i= 0; i<array.length; i++){
                    //console.log(array[i]);
                    $.get('admin/deletes_message?message_id='+array[i], function(res){
                        if(res){
                            location.href = 'admin/admin_message';
                        }
                    });
